...
SimpleNavigation::Configuration.run do |navigation|
  navigation.items do |primary|
    ...
    primary.item :administration, 'Administration', dashboard_users_url, highlights_on: %r(/admin), if: -> { can? :manage, User } do |sub_nav|
      ...
      sub_nav.item :administration_tutorials,          '<i class="fa fa-mortarboard"></i> Tutorials'.html_safe, company_tutorials_url, if: -> { can? :manage, CompanyTutorial }, highlights_on: %r(/admin/company_tutorials)
      sub_nav.item :administration_tutorial_categories,'<i class="fa fa-mortarboard"></i> Tutorial Categories'.html_safe, tutorial_categories_url, if: -> { can? :manage, TutorialCategory }, highlights_on: %r(/admin/tutorial_categories)
      sub_nav.item :administration_news_types,         '<i class="fa fa-news"></i> News Types'.html_safe, admin_news_types_url, if: -> { can? :manage, NewsType }, highlights_on: %r(/admin/admin_news_types)
    end
